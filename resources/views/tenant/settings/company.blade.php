        ...
        <div class="mb-6" x-data="{ activeTab: 'company' }">
            ...
            <div x-show="activeTab === 'modules'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100"
                 x-data="{
                    ...
                    getEnabled() {
                        return this.modules.filter(m => m.enabled).map(m => m.key);
                    },
                    toggle(mod) {
                        if (mod.core) return;
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-200 p-8 mt-6">
                    <div class="mb-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <h3 class="text-lg font-semibold text-gray-900 mb-2">Module Management</h3>
                                <p class="text-sm text-gray-600">
                                    Enable or disable modules for your business. Your business category is
                                    <span class="font-medium text-purple-600 capitalize">{{ $businessCategory }}</span>.
                                    Core modules (Dashboard, Accounting, Admin, Settings, Support, Help) cannot be disabled.
                                </p>
                            </div>
                            <form method="POST" action="{{ route('tenant.settings.company.reset-modules', ['tenant' => $tenant->slug]) }}"
                                  onsubmit="return confirm('Reset all modules to the default set for your business category?');">
                                @csrf
                                <button type="submit"
                                        class="inline-flex items-center px-4 py-2 bg-white border border-gray-300 text-gray-700 text-sm font-medium rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 transition-colors">
                                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <form method="POST" action="{{ route('tenant.settings.company.update-modules', ['tenant' => $tenant->slug]) }}">
                        @csrf
                        @method('PUT')

                        <!-- Hidden inputs for enabled modules -->
                        <template x-for="mod in modules.filter(m => m.enabled)" :key="mod.key">
                            <input type="hidden" name="modules[]" :value="mod.key">
                        </template>

                        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
                            <template x-for="mod in modules" :key="mod.key">
                                <div class="relative border rounded-xl p-4 transition-all cursor-pointer"
                                     :class="mod.enabled
                                    ...
                                    <div class="flex items-start justify-between">
                                        <div class="flex items-center space-x-3">
                                            <div class="flex items-center justify-center w-10 h-10 rounded-lg"
                                                 :class="mod.enabled ? 'bg-purple-100 text-purple-600' : 'bg-gray-200 text-gray-400'">
                                                <i :class="mod.icon" class="text-lg"></i>
                                            </div>
                                            <div>
                                                <h4 class="font-semibold text-gray-900 text-sm" x-text="mod.name"></h4>
                                        ...
                                        <div class="ml-2 flex-shrink-0">
                                            ...
                                            <div class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors"
                                                 :class="mod.enabled ? 'bg-purple-600' : 'bg-gray-300'"
                                                 x-show="!mod.core">
                                    ...
                                    <!-- Recommended badge -->
                                    <div x-show="mod.recommended && !mod.core" class="mt-2">
                                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-700">
                                            <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"/>
                                            </svg>
                                            Recommended for <span class="capitalize ml-0.5" x-text="'{{ $businessCategory }}'"></span>
                                        </span>
                                    </div>
                                </div>
                            </template>
                        ...
                        <div class="bg-gray-50 rounded-lg p-4 mb-6">
                            <p class="text-sm text-gray-600">
                                <span class="font-medium text-gray-900" x-text="modules.filter(m => m.enabled).length"></span>
                                of <span x-text="modules.length"></span> modules enabled
                            </p>
                        </div>
