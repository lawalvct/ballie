...
<div class="max-w-4xl mx-auto">
    ...
    <!-- Display validation errors summary if available -->
    @if ($errors->any())
    <div class="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-md">
        <div class="flex items-start">
            <div class="flex-shrink-0">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // -----------------------------------------------------------------
    // Duplicate vendor name / company name detection
    // -----------------------------------------------------------------
    (function () {
        const checkUrl = @json(route('tenant.crm.vendors.check-duplicate', ['tenant' => tenant()->slug]));
        const exceptId = @json($vendor->id);
        ...
        function render(el, data) {
            ...
            el.innerHTML = `
                <div class="flex items-start">
                    ...
                    <div class="flex-1">
                        <p class="font-medium">Another vendor named &ldquo;${safeName}&rdquo; already exists in this company.${emailHtml}</p>
        ...
        async function check() {
            const type = getVendorType();
            const params = new URLSearchParams({ vendor_type: type, except_id: exceptId });
            if (type === 'business') {
