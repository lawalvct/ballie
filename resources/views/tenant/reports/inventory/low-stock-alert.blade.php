...
@section('page-description')
    <span class="hidden md:inline">Products that are below reorder level or out of stock</span>
@endsection
...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-6">
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($totalAlerts) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-red-600">{{ number_format($criticalAlerts) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-yellow-600">{{ number_format($warningAlerts) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-bold text-gray-900">{{ number_format($outOfStockCount) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500">Reorder Value</p>
                    <p class="text-2xl font-bold text-gray-900">₦{{ number_format($estimatedReorderValue, 2) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <form method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div>
                ...
                <input type="date" name="as_of_date" id="as_of_date" value="{{ $asOfDate }}" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
            </div>
            <div>
                <label for="category_id" class="block text-sm font-medium text-gray-700 mb-1">Category</label>
                <select name="category_id" id="category_id" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
            ...
            <div>
                ...
                <select name="alert_type" id="alert_type" class="block w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-red-500 focus:border-red-500">
            ...
            <div class="flex items-end">
                <button type="submit" class="w-full inline-flex justify-center items-center px-4 py-2 bg-red-600 border border-transparent rounded-lg text-sm font-medium text-white hover:bg-red-700">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Low Stock Alert Details</h3>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Reorder Cost</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Status</th>
                    </tr>
                </thead>
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($paginatedProducts as $product)
                        <tr class="hover:bg-gray-50 {{ $product->alert_level === 'critical' ? 'bg-red-50' : '' }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-right">
                                <span class="font-semibold {{ $product->calculated_stock <= 0 ? 'text-red-600' : 'text-gray-900' }}">
                                    {{ number_format($product->calculated_stock, 2) }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ number_format($product->reorder_level, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-right">
                                <span class="font-semibold text-red-600">{{ number_format($product->shortage_quantity, 2) }}</span>
                                @if($product->shortage_percentage > 0)
                                    <div class="text-xs text-gray-500">({{ number_format($product->shortage_percentage, 1) }}%)</div>
                                @endif
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-semibold">
                                ₦{{ number_format($product->shortage_quantity * ($product->purchase_rate ?? 0), 2) }}
                            </td>
                        ...
                        <tr>
                            <td colspan="8" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm text-gray-500">Great! No low stock alerts at this time.</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
