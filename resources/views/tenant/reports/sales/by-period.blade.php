...
@section('title', 'Sales by Period Report')
...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-gray-900">Sales by Period Report</h1>
            <p class="text-sm text-gray-500 mt-1">Time-based sales analysis with trends and comparisons</p>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-500">Avg per Period</p>
                    <p class="text-2xl font-bold text-gray-900">₦{{ number_format($averagePerPeriod, 2) }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-lg font-bold text-gray-900">₦{{ number_format($bestPeriod['total_sales'] ?? 0, 2) }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">Sales by {{ ucfirst($periodType) }} Period</h3>
        </div>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($periodSales as $index => $period)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900 text-right">₦{{ number_format($period['total_sales'], 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ number_format($period['invoice_count']) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">₦{{ number_format($period['avg_sale'], 2) }}</td>
                            @if($compareWith)
                        ...
                        <tr>
                            <td colspan="{{ $compareWith ? 6 : 5 }}" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center">
                                    ...
                                    <p class="text-sm text-gray-500">No sales data found for the selected period</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
                </tbody>
                @if(count($periodSales) > 0)
                    <tfoot class="bg-gray-50">
                        <tr>
                            <td colspan="2" class="px-6 py-3 text-sm font-bold text-gray-900">Total</td>
                            <td class="px-6 py-3 text-sm font-bold text-gray-900 text-right">₦{{ number_format($totalSales, 2) }}</td>
                            <td class="px-6 py-3 text-sm font-bold text-gray-900 text-right">{{ number_format($totalInvoices) }}</td>
                            <td class="px-6 py-3 text-sm font-bold text-gray-900 text-right">₦{{ number_format($totalInvoices > 0 ? $totalSales / $totalInvoices : 0, 2) }}</td>
                            @if($compareWith)
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-lg border border-green-200 p-6">
                <div class="flex items-center mb-3">
                    ...
                    <h4 class="text-lg font-semibold text-green-900">Best Performing Period</h4>
                </div>
                <div class="space-y-2">
                    ...
                    <div class="flex justify-between">
                        <span class="text-sm text-green-700">Total Sales:</span>
                        <span class="text-sm font-bold text-green-900">₦{{ number_format($bestPeriod['total_sales'], 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-green-700">Invoices:</span>
                        <span class="text-sm font-semibold text-green-900">{{ number_format($bestPeriod['invoice_count']) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-gradient-to-br from-red-50 to-red-100 rounded-lg border border-red-200 p-6">
                <div class="flex items-center mb-3">
                    ...
                    <h4 class="text-lg font-semibold text-red-900">Lowest Performing Period</h4>
                </div>
                <div class="space-y-2">
                    <div class="flex justify-between">
                    ...
                    <div class="flex justify-between">
                        <span class="text-sm text-red-700">Total Sales:</span>
                        <span class="text-sm font-bold text-red-900">₦{{ number_format($worstPeriod['total_sales'], 2) }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-sm text-red-700">Invoices:</span>
                        <span class="text-sm font-semibold text-red-900">{{ number_format($worstPeriod['invoice_count']) }}</span>
                    </div>
                </div>
            </div>
        </div>
