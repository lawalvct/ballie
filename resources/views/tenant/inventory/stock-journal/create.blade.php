...
@section('title', isset($stockJournal) ? 'Edit Stock Journal Entry' : 'Create Stock Journal Entry')
@section('page-title', isset($stockJournal) ? 'Edit Stock Journal Entry' : 'Create Stock Journal Entry')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <select name="entry_type" id="entry_type" required x-model="entryType"
                        ...
                        <option value="transfer" {{ old('entry_type', $entryType ?? '') === 'transfer' ? 'selected' : '' }}>Stock Transfer</option>
                    </select>
                </div>
                ...
                <!-- Auto-Generated Journal Number (Display Only) -->
                @if(isset($stockJournal))
                <div>
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex space-x-3">
                <button type="submit" name="action" value="save"
                    ...
                    {{ isset($stockJournal) ? 'Update Entry' : 'Save as Draft' }}
                </button>
...
function journalEntryForm() {
    return {
        ...
        init() {
            ...
            @elseif(isset($duplicateFrom))
                // Load items from duplicate source
        ...
        loadExistingItems() {
            @foreach($stockJournal->items as $item)
                this.items.push({
                    ...
                    rate: {{ $item->rate }},
                    amount: {{ $item->amount }},
                    current_stock: {{ $item->stock_before }},
        ...
        loadDuplicateItems() {
            @foreach($duplicateFrom->items as $item)
                this.items.push({
                    ...
                    rate: {{ $item->rate }},
                    amount: {{ $item->amount }},
                    current_stock: 0, // Will be updated when product is selected
                    unit: '{{ $item->product->primaryUnit->name ?? "" }}',
                    batch_number: '{{ $item->batch_number ?? "" }}',
                    expiry_date: '',
                });
