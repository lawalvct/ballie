...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const exceptId = @json($ledgerAccount->id);
    const nameInput = document.getElementById('name');
    if (!nameInput) return;
    ...
    function render(data) {
        ...
        warn.innerHTML = `
            <div class="flex items-start">
                ...
                <div class="flex-1">
                    <p class="font-medium">Another ledger account named &ldquo;${safeName}&rdquo;${codeHtml} already exists in this company.${typeHtml}</p>
    ...
    async function check() {
        ...
        if (!v) { hide(); return; }
        try {
            const res = await fetch(`${checkUrl}?name=${encodeURIComponent(v)}&except_id=${encodeURIComponent(exceptId)}`, {
