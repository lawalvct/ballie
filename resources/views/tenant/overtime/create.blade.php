...
@section('title', 'Add Overtime Record - ' . $tenant->name)
@section('page-title', 'Add Overtime Record')
@section('page-description', 'Create a new overtime record for an employee')
        ...
        <form action="{{ route('tenant.overtime.store', $tenant) }}" method="POST" class="p-6 space-y-6">
            ...
            <div>
                ...
                <select id="employee_id" name="employee_id" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('employee_id') border-red-500 @enderror">
            ...
            <div id="hours_display" class="hidden bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div class="flex items-center">
                    <svg class="w-5 h-5 text-blue-600 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span class="text-sm font-medium text-blue-900">
                        Total Hours: <span id="total_hours_text" class="font-bold">0</span> hours
                    </span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="overtime_type" name="overtime_type" required
                        ...
                        <option value="weekday" {{ old('overtime_type') == 'weekday' ? 'selected' : '' }} data-multiplier="1.5">Weekday (1.5x)</option>
                        <option value="weekend" {{ old('overtime_type') == 'weekend' ? 'selected' : '' }} data-multiplier="2.0">Weekend (2.0x)</option>
                        <option value="holiday" {{ old('overtime_type') == 'holiday' ? 'selected' : '' }} data-multiplier="2.5">Holiday (2.5x)</option>
                        <option value="emergency" {{ old('overtime_type') == 'emergency' ? 'selected' : '' }} data-multiplier="2.0">Emergency (2.0x)</option>
                    </select>
                    @error('overtime_type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <!-- Amount Preview -->
            <div id="amount_preview" class="hidden bg-green-50 border border-green-200 rounded-lg p-4">
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                    ...
                    <span id="estimated_amount" class="text-lg font-bold text-green-900">₦0.00</span>
                </div>
            ...
            <div>
                ...
                <textarea id="reason" name="reason" rows="3" required
                          ...
                          class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('reason') border-red-500 @enderror">{{ old('reason') }}</textarea>
                @error('reason')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <textarea id="work_description" name="work_description" rows="3"
                          ...
                          class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-indigo-500 focus:border-transparent @error('work_description') border-red-500 @enderror">{{ old('work_description') }}</textarea>
...
function calculateHours() {
    ...
    if (startTime && endTime) {
        ...
        document.getElementById('hours_display').classList.remove('hidden');
...
function updateMultiplier() {
    const select = document.getElementById('overtime_type');
...
function calculateAmount() {
    ...
    if (startTime && endTime && hourlyRate > 0 && multiplier > 0) {
        ...
        document.getElementById('amount_preview').classList.remove('hidden');
    }
}
</script>
@endpush
