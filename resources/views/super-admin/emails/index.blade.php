...
<div class="max-w-7xl mx-auto space-y-6">
    ...
    <div class="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200 bg-gray-50">
                <h2 class="text-lg font-semibold text-gray-900">Email Accounts for {{ $domain }}</h2>
            </div>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Number of Emails</th>
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @foreach($emails as $email)
                                @php
                                    // Extract username from email address (e.g., admin@ballie.co -> admin)
                                    $emailAddress = $email['email'] ?? '';
                                    $username = strstr($emailAddress, '@', true) ?: $emailAddress;
                                @endphp
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-medium text-gray-900">{{ $emailAddress }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                        {{ $email['numberofEmails'] ?? 0 }}
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-3">
                                            <a href="{{ route('super-admin.emails.edit-password', ['domain' => $domain, 'username' => $username]) }}"
                                            ...
                                            <form method="POST" action="{{ route('super-admin.emails.destroy') }}"
            ...
            @else
                <div class="text-center py-12">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No email accounts found</h3>
                    <p class="mt-1 text-sm text-gray-500">Get started by creating a new email account for {{ $domain }}</p>
                    <div class="mt-6">
                        <a href="{{ route('super-admin.emails.create') }}"
