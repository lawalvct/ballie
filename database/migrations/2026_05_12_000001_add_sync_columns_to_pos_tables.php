...
/**
 * Phase 5 mobile offline sync — additive only.
 *
 * Adds sync metadata (sync_uuid, server_version, client_created_at,
 * client_updated_at, last_modified_by_device_id) to the POS tables so
 * mobile can:
 *   - PULL cash registers, cash register sessions, payment methods
 *     (read-only references for the offline POS screen).
 *   - PUSH sales captured offline against an already-open cash session.
 *
 * Posting (creating stock_movements + accounting voucher) happens
 * server-side during the push, mirroring the canonical web flow.
 *
