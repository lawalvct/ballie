<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

return new class extends Migration
{
    /**
     * Run the migrations.
     */
    public function up(): void
    {
        Schema::create('attendance_records', function (Blueprint $table) {
            $table->id();
            $table->foreignId('tenant_id')->constrained()->onDelete('cascade');
            $table->foreignId('employee_id')->constrained('employees')->onDelete('cascade');
            $table->date('attendance_date');

            // Clock In/Out
            $table->time('clock_in')->nullable();
            $table->time('clock_out')->nullable();
            $table->string('clock_in_ip')->nullable();
            $table->string('clock_out_ip')->nullable();
            $table->string('clock_in_location')->nullable(); // GPS coordinates or location name
            $table->string('clock_out_location')->nullable();
            $table->text('clock_in_notes')->nullable();
            $table->text('clock_out_notes')->nullable();

            // Calculated fields
            $table->time('scheduled_in')->nullable(); // Expected clock in time
            $table->time('scheduled_out')->nullable(); // Expected clock out time
            $table->integer('late_minutes')->default(0); // Minutes late
            $table->integer('early_out_minutes')->default(0); // Minutes left early
            $table->integer('work_hours_minutes')->default(0); // Total work time in minutes
            $table->integer('break_minutes')->default(0); // Break time in minutes
            $table->integer('overtime_minutes')->default(0); // Overtime in minutes

            // Status
            $table->enum('status', ['present', 'absent', 'late', 'half_day', 'on_leave', 'holiday', 'weekend'])->default('present');
            $table->text('absence_reason')->nullable();
            $table->boolean('is_approved')->default(false);
            $table->foreignId('approved_by')->nullable()->constrained('users');
            $table->timestamp('approved_at')->nullable();

            // Shift information
            $table->foreignId('shift_id')->nullable()->constrained('shift_schedules');

            // Remarks
            $table->text('remarks')->nullable();
            $table->text('admin_notes')->nullable();

            // Audit
            $table->foreignId('created_by')->nullable()->constrained('users');
            $table->foreignId('updated_by')->nullable()->constrained('users');
            $table->timestamps();
            $table->softDeletes();

            $table->unique(['employee_id', 'attendance_date'], 'emp_date_unique');
            $table->index(['tenant_id', 'attendance_date']);
            $table->index(['employee_id', 'attendance_date']);
            $table->index('status');
        });
    }

    /**
     * Reverse the migrations.
     */
    public function down(): void
    {
        Schema::dropIfExists('attendance_records');
    }
};
