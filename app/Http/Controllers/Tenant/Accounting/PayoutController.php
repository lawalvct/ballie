...
class PayoutController extends Controller
{
    ...
    public function index(Request $request, Tenant $tenant)
    {
        ...
        return view('tenant.accounting.payouts.index', compact(
            'tenant',
            'settings',
            'availableBalance',
            'stats',
    ...
    public function create(Request $request, Tenant $tenant)
    {
        ...
        if (!$settings->payouts_enabled) {
            return redirect()->route('tenant.accounting.payouts.index', $tenant->slug)
                ->with('error', 'Payouts are currently disabled by the administrator.');
        }
        ...
        if ($availableBalance < $settings->minimum_payout) {
            return redirect()->route('tenant.accounting.payouts.index', $tenant->slug)
                ->with('error', 'You need at least ₦' . number_format($settings->minimum_payout, 2) . ' to request a payout. Your current balance is ₦' . number_format($availableBalance, 2) . '.');
        }

        // Get Nigerian banks list
        $banks = $this->getNigerianBanks();

        return view('tenant.accounting.payouts.create', compact(
            'tenant',
            'settings',
            'availableBalance',
            'banks'
    ...
    public function store(Request $request, Tenant $tenant)
    {
        ...
        try {
            ...
            Log::info('Accounting payout request created', [
                'payout_id' => $payout->id,
                'tenant_id' => $tenant->id,
            ...
            return redirect()->route('tenant.accounting.payouts.show', [
                'tenant' => $tenant->slug,
                'payout' => $payout->id,
            ...
            DB::rollBack();
            Log::error('Accounting payout request failed: ' . $e->getMessage());
    ...
    public function show(Tenant $tenant, PayoutRequest $payout)
    {
        ...
        return view('tenant.accounting.payouts.show', compact('tenant', 'payout'));
    }
    ...
    /**
     * Get revenue statistics for the tenant (ledger-based).
     */
    private function getRevenueStats(Tenant $tenant): array
    {
        ...
        if ($ballieAccountId) {
            ...
            $totalCollected = $ballieAccount ? (float) $ballieAccount->current_balance : 0;
        }
        ...
        return [
            'total_collected' => $totalCollected + $totalWithdrawn,
